...
def external_eval(cur_split, test_loader, exp_res_dir=None, device="cuda", **param_kwargs):
    ...
    for graph in tqdm(test_loader):
        ...
        if param_kwargs['architecture'] in ["THItoGene", "HisToGene", "Hist2ST"]:
            if param_kwargs['architecture'] == "Hist2ST":
                ...
                proj_embed, pred_outputs = model(patches=x, centers=graph['pos'].to(device), adj=graph['adj'].to(device), return_embed=True)
    ...
    dict_split_cell_abundance_mutualinfo = {}
    for cell_idx in range(test_cell_pred_array.shape[1]):
        mutualinfo_val = mutual_info_regression(test_cell_pred_array[:, cell_idx].flatten().reshape(-1, 1), 
                                                test_cell_label_array[:, gene_cell_split_idx+cell_idx].flatten().reshape(-1,))[0]
        p = 0.0

        dict_split_cell_abundance_mutualinfo.update({f"celltype_{cell_idx}": {"mutualinfo": mutualinfo_val, "pval": p}})

    dict_slides_cell_abundance_mutualinfo = {}
    ...
    for slide_no in range(test_dataset.batch_size):
        ...
        dict_one_slide_all_celltype_mutualinfo = {}
        for cell_idx in range(test_cell_pred_array_sub.shape[1]):
            mutualinfo_val = mutual_info_regression(test_cell_pred_array_sub[:, cell_idx].flatten().reshape(-1, 1), 
                                                    test_cell_label_array_sub[:, gene_cell_split_idx+cell_idx].flatten().reshape(-1,))[0]
            p = 0.0

            dict_one_slide_all_celltype_mutualinfo.update({f"celltype_{cell_idx}": {"mutualinfo": mutualinfo_val, "pval": p}})

        dict_slides_cell_abundance_mutualinfo.update({f"id_{slide_no}": pd.DataFrame(dict_one_slide_all_celltype_mutualinfo)})

    return dict_split_cell_abundance_mutualinfo, dict_slides_cell_abundance_mutualinfo, dict_slides_spot_Predictions, test_cell_abundance_loss
...
if __name__ == "__main__":
    ...
    all_splits_mutualinfo = {}
    all_slides_mutualinfo = {}
    ...
    for cur_split in config["CKPTS"]['split_ids']:
        ...
        dict_split_cell_type_mutualinfo, dict_slides_cell_type_mutualinfo, dict_slides_spot_predictions, test_loss = external_eval(cur_split, test_loader, args.exp_path, device=device, **config["HyperParams"])

        all_splits_mutualinfo.update({cur_split: pd.DataFrame(dict_split_cell_type_mutualinfo).rename(columns=cell_type_info)})
        all_splits_loss.update({cur_split: test_loss})

        slide_name_mapping = {f"id_{idx}": test_slides[idx] for idx in range(len(test_slides))}
        dict_slides_cell_type_mutualinfo = {slide_name_mapping[slide_id]: dict_slides_cell_type_mutualinfo[slide_id] for slide_id in dict_slides_cell_type_mutualinfo.keys()}
        ...
        all_slides_mutualinfo.update(dict_slides_cell_type_mutualinfo)
        all_slides_predictions.update(dict_slides_spot_predictions)

    all_splits_mutualinfo = pd.concat(all_splits_mutualinfo.values(), keys=[name for name in all_splits_mutualinfo.keys()])

    all_splits_mutualinfo.loc["split_mean"] = (all_splits_mutualinfo.iloc[0::2, :].mean(0)) # mean along all splits
    all_splits_mutualinfo.insert(all_splits_mutualinfo.shape[1], 'celltype_mean', all_splits_mutualinfo.mean(1).values) # mean along all cell types

    all_splits_mutualinfo.to_csv(os.path.join(args.exp_path, "all_splits_all_celltypes_mutualinfo_embed.csv"))
    print(f"all_splits_mutualinfo: {all_splits_mutualinfo}")
    
    all_slides_mutualinfo = pd.concat(all_slides_mutualinfo.values(), keys=[name for name in all_slides_mutualinfo.keys()])
    all_slides_mutualinfo = all_slides_mutualinfo.rename(columns=cell_type_info)
    
    all_slides_mutualinfo.loc["slide_mean"] = (all_slides_mutualinfo.iloc[0::2, :].mean(0)) # mean along all splits
    all_slides_mutualinfo.insert(all_slides_mutualinfo.shape[1], 'celltype_mean', all_slides_mutualinfo.mean(1).values) # mean along all cell types

    all_slides_mutualinfo.to_csv(os.path.join(args.exp_path, "all_slides_all_celltypes_mutualinfo_embed.csv"))
    print(f"all_slides_mutualinfo: {all_slides_mutualinfo}")

    print(f"all_splits_loss: {all_splits_loss}")

    with open(os.path.join(args.exp_path, "all_slides_test_spot_predictions_embed_from_mutualinfopy.pkl"), "wb") as f:
    ...
    if False and "independent_list" in config["CKPTS"] and config["CKPTS"]["independent_list"] is not None:
        for independent_set in config["CKPTS"]["independent_list"]:
            ...
            all_splits_mutualinfo.to_csv(os.path.join(args.exp_path, f"all_splits_all_celltypes_mutualinfo_{independent_set}.csv"))
            print(f"all_splits_mutualinfo: {all_splits_mutualinfo}")
            ...
            all_slides_mutualinfo.to_csv(os.path.join(args.exp_path, f"all_slides_all_celltypes_mutualinfo_{independent_set}.csv"))
            print(f"all_slides_mutualinfo: {all_slides_mutualinfo}")

            print(f"all_splits_loss: {all_splits_loss}")

            with open(os.path.join(args.exp_path, f"all_slides_spot_predictions_{independent_set}_from_mutualinfopy.pkl"), "wb") as f:
                pickle.dump(all_slides_predictions, f)